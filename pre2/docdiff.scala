...
def clean(s: String) : List[String] = {
    val some_regex = "\\w+".r
    val word = some_regex.findAllIn(s).toList
    word
    }
...
def occurrences(xs: List[String]): Map[String, Int] = {
    val keys = xs.distinct
    val pairs = for (x <- keys) yield {
        (x, xs.count(_ == x))
    }
    val map = pairs.toMap
    map
...
def prod(lst1: List[String], lst2: List[String]) : Int = {
    val a = occurrences(lst1)
    val b = occurrences(lst2)
    val keys = (lst1 ++ lst2).distinct
    val prodList = for (x <- keys) yield {
        a.getOrElse(x,0)*b.getOrElse(x,0)
    }
    val prod = prodList.sum
    prod
}
...
def overlap(lst1: List[String], lst2: List[String]) : Double = {
    val numer = prod(lst1,lst2)
    val d1 = prod(lst1,lst1)
    val d2 = prod(lst2,lst2)
    val max = List(d1,d2).max
    val ans = numer.toDouble/max.toDouble
    ans
}

def similarity(s1: String, s2: String) : Double = {
    val ans = overlap(clean(s1),clean(s2))
    ans
}
