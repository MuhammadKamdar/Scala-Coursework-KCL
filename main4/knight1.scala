...
//al poKnightMoves = List((1,2), (1,-1), (0,-2), (-1,-1), (-2, 0), (-1,1), (0,2), (1,2))
...
def enum_tours(dim: Int, path: Path) : List[Path] = {
    if (path.length >= dim*dim) List(path)
      ...
      val list = for (x <- moves)yield{
        enum_tours(dim, x::path)
      }
      list.flatten
    }
}
...
def count_tours(dim: Int, path: Path) : Int = {
    if (path.length >= dim*dim) List(path).size
    else {
      val moves = legal_moves(dim, path, path(0))
      val list = for (x <- moves)yield{
        enum_tours(dim, x::path)
...
// in order to print out the time that is needed for s
// running count_tours
